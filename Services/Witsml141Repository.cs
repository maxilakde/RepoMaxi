...
namespace WitsmlODViewer.Witsml141
{
    ...
    public class Witsml141Repository : IDisposable
    {
        private readonly string _connectionString;
        private SqlConnection? _connection;

        public Witsml141Repository(string connectionString)
        {
        ...
        public void SaveWell(string uid, string? name, string? timeZone, string? statusWell,
            string? numApi, string? country, string? state, string? field, string? operatorName,
            string? dTimCreation, string? dTimLastChange, string sourceFile)
        {
                ...
                @"MERGE wells AS target
                  ...
                  WHEN MATCHED THEN
                    UPDATE SET name = @name, time_zone = @timeZone, status_well = @statusWell,
                      num_api = @numApi, country = @country, state = @state, field = @field, [operator] = @operatorName,
                      d_tim_creation = @dTimCreation, d_tim_last_change = @dTimLastChange,
                      source_file = @sourceFile, processed_at = GETDATE()
                  WHEN NOT MATCHED THEN
                    INSERT (uid, name, time_zone, status_well, num_api, country, state, field, [operator],
                      d_tim_creation, d_tim_last_change, source_file)
                    VALUES (@uid, @name, @timeZone, @statusWell, @numApi, @country, @state, @field, @operatorName,
            ...
            cmd.Parameters.AddWithValue("@numApi", (object?)numApi ?? DBNull.Value);
            ...
            cmd.Parameters.AddWithValue("@operatorName", (object?)operatorName ?? DBNull.Value);
        ...
        public void SaveTrajectory(string uid, string? wellUid, string? wellboreUid, string? name,
            ...
            string? namingSystem, string? definitive,
            string? dTimCreation, string? dTimLastChange, string sourceFile)
        {
            using var cmd = new SqlCommand(
            ...
            cmd.Parameters.AddWithValue("@namingSystem", (object?)namingSystem ?? DBNull.Value);
            cmd.Parameters.AddWithValue("@definitive", (object?)definitive ?? DBNull.Value);
            cmd.Parameters.AddWithValue("@dTimCreation", ParseDateTime(dTimCreation));
            cmd.Parameters.AddWithValue("@dTimLastChange", ParseDateTime(dTimLastChange));
            cmd.Parameters.AddWithValue("@sourceFile", (object?)sourceFile ?? DBNull.Value);
            cmd.ExecuteNonQuery();
        ...
        public IReadOnlyList<WellInfo> GetAllWells()
        {
            ...
            using var cmd = new SqlCommand(
                @"SELECT uid, name, time_zone, status_well, num_api, country, state, field, [operator],
                  d_tim_creation, d_tim_last_change, source_file, processed_at
            ...
            while (r.Read())
            {
                list.Add(new WellInfo
                {
                    ...
                    NumApi = r.IsDBNull(4) ? null : r.GetString(4),
                    Country = r.IsDBNull(5) ? null : r.GetString(5),
                    State = r.IsDBNull(6) ? null : r.GetString(6),
                    Field = r.IsDBNull(7) ? null : r.GetString(7),
                    Operator = r.IsDBNull(8) ? null : r.GetString(8),
                    DTimCreation = r.IsDBNull(9) ? null : r.GetDateTime(9),
                    DTimLastChange = r.IsDBNull(10) ? null : r.GetDateTime(10),
                    SourceFile = r.IsDBNull(11) ? null : r.GetString(11),
                    ProcessedAt = r.IsDBNull(12) ? null : r.GetDateTime(12)
                });
            }
            return list;
        }
